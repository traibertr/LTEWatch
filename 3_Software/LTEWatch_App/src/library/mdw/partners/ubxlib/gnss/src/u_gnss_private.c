                                                  ...
                                                  size_t messageBodyLengthBytes)
{
    ...
    // Message buffer for 8 + nPorts*40 for UBX-MON-COMMS (0x0a 0x36)
    char message[88] = {0};
    uint64_t y;

    if ((pInstance != NULL) &&
        (uGnssPrivateGetStreamType(pInstance->transportType) >= 0)) {
        // Send UBX-MON-COMMS to get the number of messages received
        ...
        if (errorCodeOrLength == sizeof(message)) {
            ...
            y = uUbxProtocolUint64Decode(message + ((size_t) (unsigned) (pInstance->portNumber * 40) + 28));
            ...
            if (errorCodeOrLength == messageBodyLengthBytes + U_UBX_PROTOCOL_OVERHEAD_LENGTH_BYTES) {
                ...
                if (errorCodeOrLength == sizeof(message)) {
                    ...
                    y = uUbxProtocolUint64Decode(message + ((size_t) (unsigned) (pInstance->portNumber * 40) + 28)) - y;
